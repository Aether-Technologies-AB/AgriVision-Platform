          ...
          devices.map((device) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <span
                  className={device.state ? "text-green" : "text-text-dim"}
                >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-text-dim">
                    {device.state ? "ON" : "OFF"}
                    {device.lastToggled &&
                      ` · ${new Date(device.lastToggled).toLocaleTimeString(
                        "sv-SE",
              ...
              <button
                onClick={() =>
                  sendCommand(
                    `TOGGLE_${device.type}${device.state ? "_OFF" : "_ON"}`
                  )
                }
                disabled={sending !== null || localAutoMode}
